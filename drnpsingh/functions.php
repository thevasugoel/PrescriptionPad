...
function getInvoices() {
    ...
    $query = "SELECT p.*, i.*, c.*
		...
		WHERE i.invoice = p.invoice && c.invoice = i.invoice 
	...
	if($results) {
		...
		print '<table class="table table-striped table-bordered" id="data-table" cellspacing="0"><thead><tr>
				...
				<th><h4>DOA</h4></th>
				<th><h4>DOD</h4></th>
				<th><h4>Referred By</h4></th>
				<th><h4>Type</h4></th>
				<th><h4>Hospital</h4></th>
				<th ><h4>Cash</h4></th>
				<th><h4>Cheque</h4></th>
				<th><h4>Referring Charges</h4></th>
				<th><h4>Action</h4></th>

			  </tr></thead><tbody>';

		while($row = $results->fetch_assoc()) {
$doanew = str_replace('/', '-', $row['invoice_date']);
			print '
				<tr>
					<td>'.$row["invoice"].'</td>
					<td>'.$row["name"].'</td>
				    <td>'.date("Y/m/d", strtotime($doanew)).'</td>
					...
					<td>'.$row["cashdate"].'</td>
					<td>'.$row["cheque"].'</td>
					<td>'.$row["refer_charge"].'</td>
				';

						
				if($_SESSION['login_username']=="user"){  ?>  <td><a href="invoice-edit.php?id=<?php echo $row['invoice'];?>" class="btn btn-primary btn-xs"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a> 

		<?php }else{				  print '<td><a href="invoice-edit.php?id='.$row["invoice"].'" class="btn btn-primary btn-xs"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a> <a data-invoice-id="'.$row['invoice'].'" class="btn btn-danger btn-xs delete-invoice"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a></td>
		</tr>
			';
		}
	}
